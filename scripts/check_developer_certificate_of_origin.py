...
Article 15 asks that every contributor sign off each commit, and article 16's
Guard calls the check required. Nothing in this repository read the sign-offs
until now: the rule was one sentence of `README.md`, held by whoever happened to
look, and the constitution this repository adopts by pointer described a check
that did not exist here.

    python scripts/check_developer_certificate_of_origin.py --base=<ref> --head=<ref>

Every commit in `base..head` that is not a merge must carry a `Signed-off-by:`
trailer naming a person and an address. A merge commit introduces no work of its
own, and its parents are checked where they were made, so it is passed over —
which is what the certificate itself asks for, and what refusing it would turn
into a rule about merge strategy.

The exit code is the point: `0` when every commit certifies, `1` naming each one
that does not, `2` when the range could not be read at all — a shallow checkout
has no range, and a check that cannot see the commits must not report that they
passed (article 2).

It is not a step of `scripts/gate.sh`, and that is deliberate rather than an
omission: it reads the commits of a change against the ref it started from, and
a local run has no such range to be given. The workflow hands it the two refs of
...
#: What separates one commit's report from the next in `git log`'s output.
_RECORD = "\x1e"
_FIELD = "\x1f"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if missing:
        print(
            ...
            "(article 15; `git commit -s`, or `git rebase --signoff` to repair them):",
            file=sys.stderr,
        )
        for line in missing:
