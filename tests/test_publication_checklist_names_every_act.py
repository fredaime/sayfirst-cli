...
"""Article 0: every act the constitution puts on the publication checklist is a line of it.

Three articles point at this page. Article 0 requires the marks filed before
...
sign-off check's required status a setting no check can read, and makes a fork's
ability to build with nothing but this repository the thing two workflow edits
turn true. A fourth act is a line of the page without being required of this
repository: article 18's count is published by the control plane, and the line
says where, because a reader who came here looking for it would otherwise find
nothing and conclude it is not published at all. A page that named any of them
in a paragraph rather than as lines would satisfy a reader skimming it and
...
conflicts (article 16; the rule is stated in `README.md`, which this
repository carries in place of a contributing file).
...
#: Each act the constitution names, and a phrase the line carrying it must hold.
...
ACTS: tuple[tuple[str, str], ...] = (
    ...
    (
        "the two workflow edits publication requires",
        "two workflow edits",
    ),
    (
    ...
    (
        "where the count of article 18 is published",
        "concept count",
    ),
    (
        "the dry run",
...
def test_an_act_named_in_a_paragraph_above_the_list_is_not_a_line() -> None:
    """WATCHED FIRING, on the shape a page of this project used to have. A
    paragraph naming acts « so that their absence is not read as their
    completion » is a warning to a reader and not a line for an operator, and a
    rule that read the whole page would have taken it for one. The one real line
    is required to still pass, so this proves the reading and not a refusal of
    everything."""
    page = (
